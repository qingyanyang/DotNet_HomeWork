...
namespace Net_6_Assignment.Controllers
{
    ...
    public class CategoryController : ControllerBase
    {
        ...
        public async Task<ActionResult<CategoryResponseDTO>> CreateCategory([FromBody] CategoryRequestDTO categoryInfo)
        {
            ...
            // check if duplicated
            var existingCategory = await _CategoryService.GetByConditionAsync(c => c.CategoryName == category.CategoryName);
            if(existingCategory != null)
            {
                _logger.LogWarning("The specified category already exist.");
                throw new A6ArgumentException("The specified category already exist.");
            }
            // check if parent is existing
            if (category.ParentId.HasValue) {
                var parentCategory = await _CategoryService.GetByConditionAsync(c => c.Id == category.ParentId.Value);
                if (parentCategory == null)
                {
                    _logger.LogWarning("The specified parent category does not exist.");
                    throw new A6ArgumentException("The specified parent category does not exist.");
                }
                if (parentCategory.CategoryLevel != CategoryLevel.ParentLevel)
                {
                    _logger.LogWarning("The specified parent category is not of parent level.");
                    throw new A6ArgumentException("The specified parent category is not of parent level.");
                }
            }
